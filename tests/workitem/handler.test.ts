...
describe("handleWorkitem — input validation", () => {
  ...
  it("returns 400 for missing required fields", async () => {
    ...
    expect(res.statusCode).toBe(400);
    const body = JSON.parse(res.body);
    expect(body.error).toMatch(/issueType/i);
  });
...
describe("handleWorkitem — field resolution errors", () => {
  it("returns 400 with details for not_found fields", async () => {
    mockResolveFieldNames.mockResolvedValue({
      ok: false,
      errors: [{ name: "Unknown Field", reason: "not_found" }],
    });
    ...
    expect(res.statusCode).toBe(400);
    const body = JSON.parse(res.body);
    expect(body.error).toMatch(/1 field/i);
    expect(body.details?.["Unknown Field"]).toMatch(/not found/i);
  });

  it("returns 400 with details for ambiguous fields", async () => {
    mockResolveFieldNames.mockResolvedValue({
      ok: false,
      errors: [
        {
          ...
          reason: "ambiguous",
          matches: [
            "Priority Score (customfield_10100)",
            "Priority Score (customfield_10200)",
          ],
        },
      ],
    });
    ...
    expect(res.statusCode).toBe(400);
    const body = JSON.parse(res.body);
    expect(body.details?.["Priority Score"]).toMatch(/ambiguous/i);
  });
  ...
  it("returns all errors at once (multiple failures)", async () => {
    mockResolveFieldNames.mockResolvedValue({
      ok: false,
      errors: [
        { name: "FieldA", reason: "not_found" },
        { name: "FieldB", reason: "not_found" },
        {
          name: "FieldC",
          reason: "ambiguous",
          matches: ["FieldC (cf1)", "FieldC (cf2)"],
        },
      ],
    });
    ...
    expect(res.statusCode).toBe(400);
    const body = JSON.parse(res.body);
    expect(body.error).toMatch(/3 field/i);
    expect(Object.keys(body.details ?? {})).toHaveLength(3);
  });
...
describe("handleWorkitem — successful creation", () => {
  ...
  it("omits update key when no update fields provided", async () => {
    mockResolveFieldNames.mockResolvedValue({
      ok: true as const,
      resolved: new Map([["Summary", "summary"]]),
    });
    mockCreateIssue.mockResolvedValue(CREATED_ISSUE);
...
describe("handleWorkitem — Jira API error forwarding", () => {
  ...
  it("returns 500 for unexpected errors from createIssue", async () => {
    ...
    expect(res.statusCode).toBe(500);
    const body = JSON.parse(res.body);
    expect(body.error).toMatch(/create issue/i);
  });
});
