...
describe("resolveFieldNames — success", () => {
  it("resolves a standard field by display name", async () => {
    ...
    if (!result.ok) return;
    expect(result.resolved.get("Summary")).toBe("summary");
  });
  ...
  it("resolves a custom field by display name", async () => {
    ...
    expect(result.ok).toBe(true);
    if (!result.ok) return;
    expect(result.resolved.get("Story Points")).toBe("customfield_10016");
  });
  ...
  it("resolves a field by clause name", async () => {
    ...
    expect(result.ok).toBe(true);
    if (!result.ok) return;
    expect(result.resolved.get("story_points")).toBe("customfield_10016");
  });
  ...
  it("resolves a field by its raw fieldId", async () => {
    ...
    expect(result.ok).toBe(true);
    if (!result.ok) return;
    expect(result.resolved.get("customfield_10016")).toBe("customfield_10016");
  });
  ...
  it("resolves multiple fields at once", async () => {
    ...
    expect(result.resolved.get("Summary")).toBe("summary");
    expect(result.resolved.get("Story Points")).toBe("customfield_10016");
    expect(result.resolved.get("Assignee")).toBe("assignee");
  });
...
describe("resolveFieldNames — errors", () => {
  it("returns not_found when issue type does not exist", async () => {
    const result = await resolveAs("Epic", ["Summary"]); // Epic not in fixtures
    // fallow-ignore-next-line code-duplication
    expect(result.ok).toBe(false);
    if (result.ok) return;
    expect(result.errors).toHaveLength(1);
    expect(result.errors[0].name).toBe("Epic");
    expect(result.errors[0].reason).toBe("not_found");
  });

  it("returns not_found for an unknown field name", async () => {
    const result = await resolveStory(["NonExistentField"]);
    // fallow-ignore-next-line code-duplication
    expect(result.ok).toBe(false);
    if (result.ok) return;
    expect(result.errors).toHaveLength(1);
    expect(result.errors[0].name).toBe("NonExistentField");
    expect(result.errors[0].reason).toBe("not_found");
  });
  ...
  it("collects ALL not_found errors before returning", async () => {
    ...
    expect(names).toEqual(["FieldA", "FieldB", "FieldC"]);
  });

  it("returns ambiguous when two fields share a name", async () => {
    ...
    expect(result.errors[0].matches).toHaveLength(2);
  });
  ...
  it("mixes not_found and valid results, collecting all errors", async () => {
    ...
    if (result.ok) return;
    expect(result.errors).toHaveLength(1);
    expect(result.errors[0].name).toBe("MissingField");
  });
});
