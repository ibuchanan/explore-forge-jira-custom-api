...
import type { ErrorResponse, WorkitemResponse } from "./types";
...
function json(
  statusCode: number,
  body: WorkitemResponse | ErrorResponse,
): ApiRouteResponse {
  return {
    statusCode,
...
export async function handleWorkitem(
  ...
  try {
      ...
      for (const err of result.errors) {
        details[err.name] =
          err.reason === "ambiguous"
            ? `Ambiguous — matches: ${(err.matches ?? []).join(", ")}`
            : `Field not found in project "${project}" for issue type "${issueType}"`;
      }
      return json(400, {
        error: `Could not resolve ${result.errors.length} field name(s)`,
        details,
      });
    }
    ...
    return json(500, { error: `Failed to fetch field metadata: ${message}` });
  }
  ...
  try {
    ...
    if (err instanceof JiraApiError) {
      // Forward Jira's status + body faithfully
      return {
    ...
    const message = err instanceof Error ? err.message : String(err);
    return json(500, { error: `Failed to create issue: ${message}` });
  }
}
