...
describe("Forge Architecture", () => {
  ...
  describe("transitive dependencies on @forge libraries", () => {
    ...
    it.skipIf(getAllTypeScriptFiles(join("src", "resolvers")).length === 0)(
  ...
  describe("structural rules", () => {
    ...
    it.skipIf(
      !directoryExists(join("src", "frontend")) ||
        getAllTypeScriptFiles(join("src", "resolvers")).length === 0,
    )("frontend should not import from resolvers", async () => {
    ...
    it.skipIf(
      !directoryExists(join("src", "frontend")) ||
        getAllTypeScriptFiles(join("src", "resolvers")).length === 0,
    )("resolvers should not import from frontend", async () => {
  ...
  describe("egress and fetch boundaries", () => {
    it.skipIf(
      getAllTypeScriptFiles(join("src", "resolvers")).length === 0 &&
        !directoryExists(join("src", "external")) &&
        !directoryExists(join("src", "import-lifecycle")),
    )(
